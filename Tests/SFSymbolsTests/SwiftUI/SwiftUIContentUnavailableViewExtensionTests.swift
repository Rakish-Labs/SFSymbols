    ...
    class ContentUnavailableViewExtensionTests: XCTestCase {
        /// Tests, whether the `ContentUnavailableView` retrieved via SFSymbols can be retrieved without a crash
        func testInit() {
            if #available(iOS 17.0, macOS 14.0, tvOS 17.0, watchOS 10.0, visionOS 1.0, *) {
                for symbol in TestHelper.allSymbolsWithVariants {
                    print(
                        "Testing validity of \"\(symbol.rawValue)\" via ContentUnavailableView init"
                    )
                ...
                print(
                    "To test the ContentUnavailableView initializer, iOS 17.0, macOS 14.0, tvOS 17.0, watchOS 10.0 or visionOS 1.0 is required."
                )
            }
        }
    }
