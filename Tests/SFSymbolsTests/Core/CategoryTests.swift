...
class CategoryTests: XCTestCase {
    ...
    func testCategoriesAreConsistentAcrossInstances() throws {
        ...
        XCTAssertEqual(
            instance1.has(category: .communication), instance2.has(category: .communication))
    }

    func testMultipleCategoryLookup() throws {
        let symbol = SFSymbol.quoteBubbleFill
        let expectedCategories: Set<SFSymbols.Category> = [
            .accessibility, .communication, .multicolor,
        ]
