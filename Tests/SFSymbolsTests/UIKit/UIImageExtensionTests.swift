    ...
    class UIImageExtensionTests: XCTestCase {
        ...
        func testFailingSymbols() {
            if #available(iOS 13.0, tvOS 13.0, watchOS 6.0, visionOS 1.0, *) {
                let failingSymbols = TestHelper.allSymbolsWithVariants.map { $0.rawValue }.map {
                    ($0, UIImage(systemName: $0))
                ...
                XCTFail("iOS 13, tvOS 13 or watchOS 6.0 is required to test SFSymbols.")
            }
        }

        /// Tests, whether the `UIImage` retrieved via SFSymbols is equal to the one retrieved via the `String` initializer
        func testInit() {
            if #available(iOS 13.0, tvOS 13.0, watchOS 6.0, visionOS 1.0, *) {
            ...
            if #available(iOS 13.0, tvOS 13.0, watchOS 6.0, visionOS 1.0, *) {
                ...
                for symbol in TestHelper.allSymbolsWithVariants {
                    for configuration in configurations {
                        print(
                            "Testing validity of \"\(symbol.rawValue)\" with configuration \"\(configuration)\" via UIImage init"
                        )

                        let expected = UIImage(
                            systemName: symbol.rawValue, withConfiguration: configuration)
