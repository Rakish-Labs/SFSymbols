@testable import SFSymbols
    ...
    final class UIApplicationShortcutIconExtensionTests: XCTestCase {
        /// Tests, whether the `UIApplicationShortcutIcon` retrieved via SFSymbols is equal to the one retrieved via the `String` initializer
        func testInit() {
            if #available(iOS 13.0, visionOS 1.0, *) {
                for symbol in TestHelper.allSymbolsWithVariants {
                    print(
                        "Testing validity of \"\(symbol.rawValue)\" via UIApplicationShortcutIcon init"
                    )
                ...
                XCTFail("iOS 13 is required to test SFSymbols.")
            }
        }
        ...
        func testTestMechanism() {
            if #available(iOS 13.0, visionOS 1.0, *) {
                let expected = UIApplicationShortcutIcon(
                    systemImageName: TestHelper.sampleSymbolRawValue)
                let actual = UIApplicationShortcutIcon(systemSymbol: TestHelper.sampleSymbol)
                let wrong = UIApplicationShortcutIcon(
                    systemSymbol: TestHelper.sampleSymbolWrongDerivate)
